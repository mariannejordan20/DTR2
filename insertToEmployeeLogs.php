...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_status = "SELECT Employee_Status FROM employee_information WHERE Employee_ID = ?";
    $stmt_check_status = $conn->prepare($sql_check_status);
    $stmt_check_status->bind_param("s", $employeeID);
    ...
    if ($result_check_status->num_rows > 0) {
        ...
        if ($employeeStatus == 1) {
            // If employee status is active, proceed with time in/out operations
            ...
            // If employee status is inactive, send an error response
            echo "Error: Your account is inactive. You are unable to timestamp.";
            exit;
        } else {
            // If employee status is neither active nor inactive, send an error response
            echo "Error: Invalid employee status.";
            exit;
        }
    } else {
        // If no employee found with the given ID, send an error response
